---
title: "Explaining fear of being a crime victim"
output:
  rmarkdown::html_vignette:
    ...
    fig_height: 6
vignette: >
  %\VignetteIndexEntry{Explaining fear of being a crime victim}
...
```{r}
# keep only entries with value < 3 
my_data <- lapop.2014.HND$vic40[lapop.2014.HND$vic40<3]
...
```{r}
# stats::binom.test(sum(my_data==1),
#                   length(my_data))
```
...
```{r}
stats::binom.test(sum(head(my_data,100)==1),
                  100)
```
...
ggplot(f,aes(factor(ans,labels=c('Yes','No')))) + 
  geom_bar(fill = unhcRstyle::unhcr_blue) +
  ...
  coord_flip() +
  labs(title = "Out of fear of being a crime victim", 
             subtitle = "In the last 12 months have you limited the places where you go shop?",
             x = " ", 
             y = "",
...
ggplot(plotme,aes(x=vic1ext,y=vic40)) + 
  ...
  annotate('text',label='Changed shopping',  size=7,x=1,y=1.4,hjust=0.5,vjust=0) +
  annotate('text',label="Didn't change", size=7,x=1.5,y=1.4,hjust=0.5,vjust=0) +
  annotate('text',label="Crime victim",     size=7,x=0.75,y=0,hjust=1,vjust=0) +
  annotate('text',label="Non-victim", size=7,x=0.75,y=1,hjust=1,vjust=0) +
...
ISO_3166_2$prov <- ISO_3166_2$Code # need matching names for join operation
joined <- join(ISO_3166_2,map_avg,by="prov")[c('Code','Name','vic40')]

joined <- joined[ !(is.na(joined$vic40)), ]

g <- googleVis::gvisGeoChart(joined,locationvar='Code',
                             colorvar='vic40',
                             hovervar='Name',
                             options=list(region="HN",
