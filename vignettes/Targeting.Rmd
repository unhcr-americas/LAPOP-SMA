...
knitr::opts_chunk$set(fig.width = 5.5, fig.height = 4, fig.align = "center", dev = "ragg_png", fig.retina = 2)
```
...
```{r fit}
mdata <- mdata %>% mutate(pred_pcinc = predict(m) %>% invBoxCox(lambda = rec %>% tidy(1) %>% pull(value)))

pline <- quantile(mdata$pcinc, .4)
thresh <- pline

mdata <- mdata %>% mutate(class = case_when((pcinc > pline) & (pred_pcinc < thresh) ~ "Inclusion error",
                                            (pcinc < pline) & (pred_pcinc > thresh) ~ "Exclusion error"))
...
mdata %>% 
  ...
  annotate("text", label = glue::glue("inclusion error\n({scales::label_percent(accuracy = .01)(mean((mdata$pcinc > pline) & (mdata$pred_pcinc < thresh)))})"), 
           x = max(log(mdata$pcinc)), y = min(log(mdata$pred_pcinc)), 
           hjust = 1, vjust = 0) +
  annotate("text", label = glue::glue("exclusion error\n({scales::label_percent(accuracy = .01)(mean((mdata$pcinc < pline) & (mdata$pred_pcinc > thresh)))})"), 
           x = min(log(mdata$pcinc)), y = max(log(mdata$pred_pcinc)), 
           hjust = 0) +
  coord_fixed() +
  labs(x = "income per capita\nlog(observed)", y = "income per capita\nlog(predicted)",
       color = NULL,
       title = "PMT Model",
       subtitle = "Model Fit",
       caption = glue::glue("R-squared = {round(rsq_vec(augment(m)$pcinc, augment(m)$.fitted), 3)}",
                            "RMSE = {round(rmse_vec(augment(m)$pcinc, augment(m)$.fitted), 3)}",
                            .sep = "\n")) +
  theme_minimal(base_family = "Lato") +
...
tradeoffs <- 
  ...
  head(-1) %>% 
  tail(-1) %>% 
  mutate(vals = map(pcinc, ~optimum_threshold(mdata, pcinc, pred_pcinc, .))) %>%
  unnest(vals)

tradeoffs %>% #
  transmute(pcinc_percentile = percent_rank(pcinc),
            # .threshold = map_dbl(.threshold, ~mean(.>=pcinc)),
            coverage = sensitivity,
            leakage = 1-specificity) %>% 
  pivot_longer(-pcinc_percentile, names_to = "measure", values_to = "value") %>% 
  ggplot() +
  geom_smooth(aes(pcinc_percentile, value, color = measure), span = .75) +
  geom_vline(xintercept = .4) +
  geom_abline() +
  annotate("text", x = .4, y = 1, label = "National Poverty Line", hjust = -.05) +
