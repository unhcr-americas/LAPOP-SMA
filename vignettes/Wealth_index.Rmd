...
vignette: >
  %\VignetteIndexEntry{Wealth index}
...
All of our p-values on logistic regressions are highly significant, meaning that our wealth index has a strong influence on all of these wealth-related variables. This is what we would expect! 

Notice that the smallest p-values (i.e. the highest significance) are for variables like `r14` and `r18` where the influence on the wealth index was higher. This is also what we would expect; variables like `r8` are still significant but not as strong.
...
which of the variables in our wealth index have historical information available, and how these vary over time.
