...
vignette: >
  %\VignetteIndexEntry{Satisfaction with Democracy}
...
```{r satisdem, echo=F, fig.width=13, fig.height=11}
Data %>%
  ...
  ggplot(aes(x=factor(satisdem), y =perc, fill=`Party ID`)) + theme_steve() +
...
Data %>%
  ...
  mutate(perc = round(sum/sum(sum), 2),
         `Party ID` = pid3,
         percper = paste0(perc*100,"%")) %>%
  ggplot(aes(x=factor(usdemocratic), y =perc, fill=`Party ID`)) + theme_steve() +
  geom_bar(stat="identity", position="dodge", color="black") + # aes(fill=`Party ID`), position=position_dodge(),
  facet_wrap(~ year, ncol=2, scales="free") +
  scale_y_continuous(labels = scales::percent, limits=c(0,1)) +
...
Data %>%
    ...
    mutate(perc = round(sum/sum(sum), 2),
           `Party ID` = pid3,
           percper = paste0(perc*100,"%")) %>%
    ggplot(aes(x=factor(potus_limitopp), y =perc, fill=`Party ID`)) + theme_steve() +
    geom_bar(stat="identity", position="dodge", color="black") + # aes(fill=`Party ID`), position=position_dodge(),
    facet_wrap(~ year, ncol=2, scales="free") +
  scale_y_continuous(labels = scales::percent, limits=c(0,1)) +
...
Data %>%
    ...
    mutate(perc = round(sum/sum(sum), 2),
           `Party ID` = pid3,
           percper = paste0(perc*100,"%")) %>%
    ggplot(aes(x=factor(closecong), y =perc, fill=`Party ID`)) + theme_steve() +
    geom_bar(stat="identity", position="dodge", color="black") + # aes(fill=`Party ID`), position=position_dodge(),
    facet_wrap(~ year, ncol=2, scales="free") +
  scale_y_continuous(labels = scales::percent, limits=c(0,1)) +
